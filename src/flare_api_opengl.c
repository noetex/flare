	...
	#define OPENGL_PLATFORM_MODULE(...)
#elif defined FLARE_SYSTEM_WINDOWS
typedef HMODULE module_handle;

void opengl_platform_destroy(void*);
void opengl_platform_init(void*, char**);
void* opengl_platform_query_function(char*);

#define QUERY_OPENGL_FUNCTION(Name) Name = opengl_platform_query_function(#Name);
...
static opengl_version_info
opengl_getversion(void)
{
	opengl_version_info Result = {0};
	...
	if(glGetIntegerv)
	{
		...
		glGetIntegerv(GL_CONTEXT_PROFILE_MASK, &Result.Profile);
		return Result;
	}
	QUERY_OPENGL_FUNCTION(glGetString);
	if(glGetString)
	{
		char OpenGL_ES[] = "OpenGL ES ";
		char OpenGL_ESCL[] = "OpenGL ES-CL ";
		char OpenGL_ESCM[] = "OpenGL ES-CM ";
		char* Version = glGetString(GL_VERSION);
		if(memory_equal(Version, OpenGL_ES, sizeof(OpenGL_ES)))
		{
			//Result.Profile = GL_PROFILE_ES;
			Version += sizeof(OpenGL_ES);
		}
		else if(memory_equal(Version, OpenGL_ESCL, sizeof(OpenGL_ESCL)))
		{
			Version += sizeof(OpenGL_ESCL);
		}
		else if(memory_equal(Version, OpenGL_ESCM, sizeof(OpenGL_ESCM)))
		{
			Version += sizeof(OpenGL_ESCM);
		}
...
#define OPENGL_VERSION_GREATER_OR_EQUAL(Major, Minor, MinMajor, MinMinor) (((Major) > (MinMajor)) || (((Major) == (MinMajor)) && (Minor) >= (MinMinor)))

static void
opengl_load_core(void* Module, opengl_version Version)
{
	if(OPENGL_VERSION_GREATER_OR_EQUAL(Verion, 1, 0))
	{
	...
	if(OPENGL_VERSION_GREATER_OR_EQUAL(Verion, 1, 1))
	{
	...
	if(OPENGL_VERSION_GREATER_OR_EQUAL(Verion, 1, 2))
	{
	...
	if(OPENGL_VERSION_GREATER_OR_EQUAL(Verion, 1, 3))
	{
	...
	if(OPENGL_VERSION_GREATER_OR_EQUAL(Verion, 1, 4))
	{
	...
	if(OPENGL_VERSION_GREATER_OR_EQUAL(Verion, 1, 5))
	{
	...
	if(OPENGL_VERSION_GREATER_OR_EQUAL(Verion, 2, 0))
	{
	...
	if(OPENGL_VERSION_GREATER_OR_EQUAL(Verion, 2, 1))
	{
	...
	if(OPENGL_VERSION_GREATER_OR_EQUAL(Verion, 3, 0))
	{
	...
	if(OPENGL_VERSION_GREATER_OR_EQUAL(Verion, 3, 1))
	{
	...
	if(OPENGL_VERSION_GREATER_OR_EQUAL(Verion, 3, 2))
	{
	...
	if(OPENGL_VERSION_GREATER_OR_EQUAL(Verion, 3, 3))
	{
	...
	if(OPENGL_VERSION_GREATER_OR_EQUAL(Verion, 4, 0))
	{
	...
	if(OPENGL_VERSION_GREATER_OR_EQUAL(Verion, 4, 1))
	{
	...
	if(OPENGL_VERSION_GREATER_OR_EQUAL(Verion, 4, 2))
	{
	...
	if(OPENGL_VERSION_GREATER_OR_EQUAL(Verion, 4, 3))
	{
	...
	if(OPENGL_VERSION_GREATER_OR_EQUAL(Verion, 4, 4))
	{
	...
	if(OPENGL_VERSION_GREATER_OR_EQUAL(Verion, 4, 5))
	{
	...
	if(OPENGL_VERSION_GREATER_OR_EQUAL(Verion, 4, 6))
	{
...
static bool_t
opengl_load_es(void* Module)
{

}

static bool_t
opengl_load_extensions(void* Module)
{
...
opengl_load(void)
{
	if(!opengl_platform_context_init());
	...
	void* OpenGLModule = OPENGL_PLATFORM_MODULE();
	opengl_version Version = opengl_getversion();
	opengl_load_core(OpenGLModule, Version);
	opengl_load_extensions(OpenGLModule);
